...
class weapon:
    def __init__(self, uuid: str, cost: int, discount: int = 0, discountPersentage: int = 0):
        ...
        with open('assets/dict/zh_TW.json', encoding='utf8') as f:
            ...
            f.close()
        self.name = requests.get(f'https://valorant-api.com/v1/weapons/skinlevels/{self.uuid}?language=zh-TW', timeout=30).json()['data']['displayName']
        self.uid = data[self.name]  # the real series skin uuid for the weapon, not a level uuid
        self.data = requests.get(f'https://valorant-api.com/v1/weapons/skins/{self.uid}?language=zh-TW', timeout=30).json()['data']
